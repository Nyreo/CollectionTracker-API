...
import {
  loginConfig,
  registerConfig,
} from "../interfaces/request_interfaces.ts";
...
export async function login(credentials: loginConfig) {
  ...
  if (!user) throw new Error("A user with that name does not exist.");
    ...
    console.log(
      `Comparing passwords: ${credentials.password},${user.password}`,
    );
    const pass = user.password;
    const valid = await compare(credentials.password, pass);

    if (!valid) {
      throw new Error(`Invalid password for user: ${credentials.username}.`);
    }
  }
...
export async function register(credentials: registerConfig) {
  ...
  if (insertId) {
    console.log(`Succesfully registered the user: ${credentials.username}`);
