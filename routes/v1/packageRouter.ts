...
import { getPackages, patchPackage, postPackage } from "./modules/packages.ts";
  ...
  router
    ...
    .patch(SUB_ROUTE, async (context) => {
      ...
      try {
        ...
        if (!params.trackingnumber) {
          throw new Error("Tracking number was not provided");
        }

        // check length
        if (params.trackingnumber.length !== 24) {
          throw new Error("Tracking number is not the correct length.");
        }
        ...
        const _package = await patchPackage(
